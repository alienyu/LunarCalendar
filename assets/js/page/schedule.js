            ...
            for(var j=0;j<scheduleData[i].events.length;j++){
                if(scheduleData[i].events[j].isOwner){
                    if(scheduleData[i].events[j].joiners[0]){
                        ...
                        eventList += template2.replace(/{{eventId}}/g,scheduleData[i].events[j].event.eventId).replace(/{{name}}/g,scheduleData[i].events[j].event.name).replace(/{{time}}/g,Dom.getHourMinute(scheduleData[i].events[j].event.startTime)).replace(/{{count}}/g,"<span>"+joinerNum+"</span><span>人</span>").replace(/{{user}}/g,"@"+user);
                    }else{
                        eventList += template2.replace(/{{eventId}}/g,scheduleData[i].events[j].event.eventId).replace(/{{name}}/g,scheduleData[i].events[j].event.name).replace(/{{time}}/g,Dom.getHourMinute(scheduleData[i].events[j].event.startTime)).replace(/{{count}}/g,"").replace(/{{user}}/g,"");
                    }
                }else{
                    joinerNum = parseInt(scheduleData[i].events[j].joiner.length)+1;
