...
//获取一天的事件
function getEventOfDay(dateTime) {
...
//设置生日获取私人黄历
function setBirthday(birthdayTime) {
    var date = new Date();
...
//添加事件页面数据提交
function eventAdd(name,tagName, startTime, endTime, tipType, tipTime, repeatType, location, remark) {
    $.ajax({
        type: "post",
...
//添加事件页面数据提交
function eventAdd2(name,tagName,startTime, endTime, tipType, tipTime, repeatType, location, remark) {
    $.ajax({
        type: "post",
        url: "http://www.li-li.cn/llwx/event/add",
        data: {
